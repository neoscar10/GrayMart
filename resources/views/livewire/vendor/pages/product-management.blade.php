...
<div class="container-fluid py-4">
    ...
    <div class="d-flex flex-wrap gap-2 align-items-center justify-content-between mb-3">
        ...
        <div class="d-flex flex-wrap gap-2 align-items-center">
            ...
            {{-- Keep your existing quick category filter as a simple select --}}
    ...
    if ($p->variants_count > 0) {
        ...
        $max = $p->variants->max(fn($v) => $v->price ?? $p->price);
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                @if($p->variants_count > 0 && $min !== null && $max !== null)
                                    @if($min === $max)
                            ...
                            <td class="text-end">
                                <div class="btn-group">
                                    ...
                                    <a class="btn btn-sm btn-outline-primary text-nowrap"
                                        href="{{ route('vendor.auctions.index', ['product' => $p->id]) }}">
                                        <i class="fa-solid fa-gavel"></i> Auctions
                                    </a>
                                    <button
                                        class="btn btn-sm btn-outline-{{ $p->is_active ? 'warning' : 'success' }} text-nowrap"
    ...
    <div wire:ignore.self class="modal fade" id="productModal" tabindex="-1" style="color:black;">
        <div class="modal-dialog modal-xl modal-dialog-centered modal-fullscreen-md-down">
            {{-- Important: stable key so form re-renders when switching products --}}
            <div class="modal-content" wire:key="product-modal-{{ $editingId ?? 'new' }}">
                <form wire:submit.prevent="save">
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            ...
                            {{-- >>> REPLACED: Category SELECT -> Command Palette Picker <<< --}}
                            <div class="col-md-5 position-relative">
                                <label class="form-label">Category <span class="text-danger">*</span></label>

                                @if($category_id)
                                    @php
    $sel = collect($flatCategories)->firstWhere('id', (int) $category_id);
                                    @endphp
                                ...
                                <div class="row g-3 align-items-center">
                                    <div class="col-md-4">
                                        ...
                                        <small class="text-muted d-block">
                                            ...
                                            @else
                                                When enabled, product price is hidden and each variant requires a price.
                                            @endif
                                        </small>
                                    </div>
                            ...
                            {{-- Single price (only when NOT using variants AND NOT reserved) --}}
                            ...
                            {{-- Auction fields (only when reserved) --}}
                            @if($is_reserved)
                                <div class="col-12">
                                    <hr>
                                    <h5 class="mb-2">Auction Pricing</h5>
                                    <small class="text-muted d-block mb-2">
                                        Base price and variant pricing are disabled for reserved products. Use these fields
                                        instead.
                                    </small>
                                </div>
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    <label class="form-label">Minimum Bid Increment ($) <span
                            ...
                            <div class="col-md-6">
                                <label class="form-label">
                                    ...
                                    <span class="text-muted small">(1–8 images)</span>
                                </label>
                                ...
                                @error('newImages.*') <small class="text-danger d-block">{{ $message }}</small>
                                @enderror
                            ...
                            @if($is_signed)
                                <div class="col-md-6">
                                    <label class="form-label">
                                        Upload Certificate (PDF) <span class="text-danger">*</span>
                                    </label>
                                    <input type="file" class="form-control" wire:model="certificateFile"
                            ...
                            {{-- Variants (only when using variants and NOT reserved) --}}
                            @if($use_variants && !$is_reserved)
                                <div class="col-12">
                                    <hr>
                                    ...
                                    @if(empty($variants))
                                        <div class="text-muted">No variants yet. Add one to set per-variant price/stock and
                                            attributes.</div>
                                    @endif

                                    @foreach($variants as $i => $v)
                                        <div class="border rounded p-3 mb-2"
                                            wire:key="variant-row-{{ $v['id'] ?? ('new-' . $i) }}">
                                            <div class="row g-2">
                                                <div class="col-md-4">
                                                    <label class="form-label">Price ($) <span
                                                            class="text-danger">*</span></label>
                                                    <input type="number" step="0.01" class="form-control"
                                                        wire:model.live="variants.{{ $i }}.price">
                                                    @error("variants.$i.price") <small
                                                    class="text-danger">{{ $message }}</small> @enderror
                                                </div>
                                                <div class="col-md-4">
                                                    <label class="form-label">Stock</label>
                                                    <input type="number" class="form-control"
                                                        wire:model.live="variants.{{ $i }}.stock">
                                                    @error("variants.$i.stock") <small
                                                    class="text-danger">{{ $message }}</small> @enderror
                                                </div>
                                                ...
                                                {{-- Attribute Values, grouped by attribute --}}
                                                <div class="col-12">
                                                    <div class="row g-2">
                            ...
                            <div class="col-12">
                                <div class="alert alert-warning d-flex align-items-center mb-0">
                                    ...
                                    <div>
                                        Any changes you save will set this product’s status to <strong>Pending</strong>
                                        for admin approval.
                                    </div>
                                </div>
                            </div>
    ...
    <div wire:ignore.self class="modal fade" id="bulkModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-footer justify-content-center">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-primary" wire:click="runBulkAction"
                        wire:loading.attr="disabled">
...
@push('scripts')
    {{-- Sortable: native HTML5 drag & drop (no external deps) --}}
    <script>
